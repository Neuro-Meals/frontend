...
<div x-data="driverManager()" x-init="init()" class="space-y-4">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-all cursor-pointer group" @click="showDriver(driver.id)">
                ...
                <div class="flex items-center justify-between pt-3 border-t border-gray-50">
                    <div class="flex items-center gap-2">
                        <button @click.stop="editDriver(driver)" class="text-[10px] font-bold text-[#6E7A25] hover:underline">{{ __('Edit') }}</button>
                        <span class="text-gray-200">·</span>
                        <button @click.stop="confirmDelete(driver)" class="text-[10px] font-bold text-red-500 hover:underline">{{ __('Deactivate') }}</button>
                    </div>
    ...
    <div x-show="deleteOpen" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        <div x-show="deleteOpen" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="absolute inset-0 bg-gray-900/50 backdrop-blur-sm" @click="deleteOpen = false"></div>
        <div x-show="deleteOpen" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h3 class="text-sm font-bold text-gray-900">{{ __('Deactivate Driver') }}</h3>
                    <p class="text-xs text-gray-500" x-text="deleteTarget ? deleteTarget.name : ''"></p>
                </div>
            </div>
            <p class="text-xs text-gray-600 mb-5">{{ __('Are you sure you want to deactivate this driver? They will no longer be able to log in or receive deliveries.') }}</p>
            <div class="flex items-center justify-end gap-2">
                <button @click="deleteOpen = false" class="px-4 py-2 rounded-lg border border-gray-200 text-xs font-bold text-gray-600 hover:bg-gray-50">{{ __('Cancel') }}</button>
                <button @click="deleteDriver()" :disabled="deleting" class="px-4 py-2 rounded-lg bg-red-500 text-white text-xs font-bold hover:bg-red-600 disabled:opacity-60">
                    <span x-show="!deleting">{{ __('Deactivate') }}</span>
                    <span x-show="deleting">{{ __('Deactivating...') }}</span>
                </button>
            </div>
        </div>
    </div>
        ...
        async deleteDriver() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    this.success = data.message || '{{ __('Driver deactivated successfully.') }}';
                    setTimeout(() => { this.success = ''; }, 3000);
                } else {
                    this.error = data.message || '{{ __('Failed to deactivate driver.') }}';
                }
            } catch (e) {
