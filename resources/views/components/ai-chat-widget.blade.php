...
<div id="{{ $wrapperId }}" class="ai-chat-widget fixed {{ $positionClass }} bottom-4 z-50 w-full max-w-sm font-sans"
    data-endpoint="{{ $resolvedEndpoint }}" data-context="{{ $context }}"
    data-error-process="{{ __('Sorry, I could not process that. Please try again.') }}"
    data-error-network="{{ __('Sorry, I\'m having trouble connecting right now. Please try again later.') }}"
    dir="{{ $isRtl ? 'rtl' : 'ltr' }}" lang="{{ app()->getLocale() }}">
    {{-- Toggle Button --}}
    <button type="button"
        class="ai-chat-toggle relative w-14 h-14 rounded-full bg-gradient-to-br from-[#173327] to-[#6E7A25] text-white shadow-2xl shadow-[#6E7A25]/40 flex items-center justify-center hover:scale-110 active:scale-95 transition-all duration-300 ml-auto ai-chat-float ai-chat-pulse"
        aria-label="{{ __('Open AI chat') }}">
        <img src="{{ $logoUrl }}" alt="Nutrio AI" class="w-9 h-9 object-contain">
    </button>

    {{-- Chat Window --}}
    <div class="ai-chat-window hidden absolute bottom-16 {{ $position === 'bottom-left' ? 'left-0' : 'right-0' }} w-full bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden flex flex-col origin-bottom-right {{ $isRtl ? 'rtl' : '' }}"
        style="height: 460px;" dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
        {{-- Header --}}
        <div
            class="bg-gradient-to-r from-[#173327] to-[#6E7A25] text-white p-4 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-full bg-white/20 backdrop-blur flex items-center justify-center border border-white/20">
            ...
            <button type="button"
                class="ai-chat-close w-8 h-8 rounded-full hover:bg-white/20 flex items-center justify-center transition-colors"
        ...
        <div class="ai-chat-messages flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-gray-900/50">
            <div class="ai-message flex gap-2.5">
                <div
                    class="w-8 h-8 rounded-full bg-gradient-to-br from-[#173327] to-[#6E7A25] flex items-center justify-center flex-shrink-0 border border-white/10 shadow-sm">
                    <img src="{{ $logoUrl }}" alt="AI" class="w-5 h-5 object-contain">
                </div>
                <div
                    class="ai-msg-bot bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-3 shadow-sm text-sm text-gray-700 dark:text-gray-200 max-w-[82%]">
                    {{ $resolvedGreeting }}
                </div>
            </div>
        </div>
        ...
        {{-- Input --}}
        <form
            class="ai-chat-form p-3 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700 flex gap-2 shrink-0">
            <div class="flex-1 relative">
                <input type="text" name="message"
                    class="ai-chat-input w-full pl-4 pr-10 py-3 rounded-2xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-[#6E7A25]/50 focus:border-[#6E7A25] transition-all"
                    placeholder="{{ $resolvedPlaceholder }}" autocomplete="off" required>
            </div>
            <button type="submit"
                class="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#173327] to-[#6E7A25] text-white flex items-center justify-center shadow-lg shadow-[#6E7A25]/25 hover:shadow-xl hover:scale-105 active:scale-95 transition-all shrink-0"
                aria-label="{{ __('Send message') }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="currentColor" viewBox="0 0 16 16">
                    <path
                        d="M15.964.686a.5.5 0 0 0-.65-.65L.767 5.855H.766l-.452.18a.5.5 0 0 0-.082.887l.41.26.001.002 4.995 3.178 1.59 2.498A.5.5 0 0 0 7 12.5v-3.768l6.546-2.969a.5.5 0 0 0 .063-.768l-6.545-2.97v-3.77l5.48 8.605Z" />
                </svg>
            </button>
        </form>
    </div>
...
(function() {
    ...
    widgets.forEach(widget => {
        ...
        const errorProcess = widget.dataset.errorProcess ||
            'Sorry, I could not process that. Please try again.';
        const errorNetwork = widget.dataset.errorNetwork ||
