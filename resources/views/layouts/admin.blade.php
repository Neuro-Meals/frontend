    ...
    <aside id="adminSidebar" class="fixed top-0 left-0 rtl:left-auto rtl:right-0 z-50 w-64 h-screen bg-brand-700 transform -translate-x-full rtl:translate-x-full rtl:-translate-x-full lg:translate-x-0 rtl:lg:translate-x-0 transition-transform duration-300 flex flex-col">
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
            ...
            <div x-data="{ open: {{ request()->routeIs('admin.reports.*') ? 'true' : 'false' }} }" class="space-y-1">
                <button @click="open = !open" class="nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('admin.reports.*') ? 'active' : '' }}">
                    ...
                    <svg class="w-4 h-4 flex-shrink-0 text-accent-400 transition-transform duration-200" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 max-h-0" x-transition:enter-end="opacity-100 max-h-screen" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 max-h-screen" x-transition:leave-end="opacity-0 max-h-0" class="overflow-hidden space-y-1 pl-4">
                    <a href="{{ route('admin.reports.dashboard') }}" class="nav-item w-full flex items-center gap-3 px-3 py-2 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('admin.reports.dashboard') ? 'active' : '' }}">
                        <svg class="w-4 h-4 flex-shrink-0 text-accent-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z"/></svg>
                        <span>{{ __('Report Overview') }}</span>
                    </a>
                    <a href="{{ route('admin.reports.revenue') }}" class="nav-item w-full flex items-center gap-3 px-3 py-2 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('admin.reports.revenue') ? 'active' : '' }}">
                        <svg class="w-4 h-4 flex-shrink-0 text-accent-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        <span>{{ __('Revenue & Finance') }}</span>
                    </a>
                    <a href="{{ route('admin.reports.delivery') }}" class="nav-item w-full flex items-center gap-3 px-3 py-2 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('admin.reports.delivery') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('admin.reports.subscriptions') }}" class="nav-item w-full flex items-center gap-3 px-3 py-2 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('admin.reports.subscriptions') ? 'active' : '' }}">
                        <svg class="w-4 h-4 flex-shrink-0 text-accent-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        <span>{{ __('Subscription & Retention') }}</span>
                    </a>
                    <a href="{{ route('admin.reports.notifications') }}" class="nav-item w-full flex items-center gap-3 px-3 py-2 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('admin.reports.notifications') ? 'active' : '' }}">
                        <svg class="w-4 h-4 flex-shrink-0 text-accent-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"/></svg>
                        <span>{{ __('Notifications & Campaign') }}</span>
                    </a>
                    <a href="{{ route('admin.reports.audit') }}" class="nav-item w-full flex items-center gap-3 px-3 py-2 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('admin.reports.audit') ? 'active' : '' }}">
