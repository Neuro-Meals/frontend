...
<body class="font-['Nunito',sans-serif] antialiased bg-gray-50 text-slate-800">
    ...
    {{-- Sidebar --}}
    <aside id="userSidebar" class="fixed top-0 left-0 rtl:left-auto rtl:right-0 z-50 w-64 h-screen bg-brand-700 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 flex flex-col">
        {{-- Brand --}}
        <div class="h-16 flex items-center justify-between px-6 border-b border-brand-800/50 flex-shrink-0">
            <img src="{{ asset('blackmodelogo.png') }}" alt="Nutrio Meals" class="h-9 w-auto">
            <button onclick="closeSidebar()" class="lg:hidden text-brand-300/60 hover:text-white transition-colors p-1 rounded-lg hover:bg-white/5">
        ...
        <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
            ...
            <div class="sidebar-group">
                <a href="{{ route('user.dashboard') }}" onclick="closeSidebar()" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('user.dashboard') ? 'active' : '' }}">
            ...
            {{-- Subscriptions --}}
            <div class="sidebar-group">
                <a href="{{ route('user.subscriptions') }}" onclick="closeSidebar()" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('user.subscriptions*') ? 'active' : '' }}">
            ...
            {{-- My Meals --}}
            <div class="sidebar-group">
                <a href="{{ route('user.meals') }}" onclick="closeSidebar()" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('user.meals*') ? 'active' : '' }}">
            ...
            {{-- Nutrition Tracker --}}
            <div class="sidebar-group">
                <a href="{{ route('user.nutrition') }}" onclick="closeSidebar()" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('user.nutrition*') ? 'active' : '' }}">
            ...
            {{-- Orders --}}
            <div class="sidebar-group">
                <a href="{{ route('user.orders') }}" onclick="closeSidebar()" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('user.orders*') ? 'active' : '' }}">
            ...
            {{-- Delivery --}}
            <div class="sidebar-group">
                <a href="{{ route('user.delivery') }}" onclick="closeSidebar()" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-brand-100 text-sm font-medium {{ request()->routeIs('user.delivery*') ? 'active' : '' }}">
        ...
        <div class="p-4 border-t border-brand-800/50">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-semibold text-white truncate">{{ $userName ?: 'User' }}</p>
    ...
    <div class="lg:ml-64 rtl:lg:mr-64 rtl:lg:ml-0 min-h-screen flex flex-col">
        ...
        {{-- Page Content --}}
        <main class="flex-1 p-4 sm:p-6 animate-fade pb-24 lg:pb-6">
            @yield('content')
        </main>
    ...
    <div class="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-lg border-t border-gray-200/80 lg:hidden shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        <div class="flex items-center justify-around py-2 px-2 max-w-lg mx-auto">
            <a href="{{ route('user.dashboard') }}" class="mobile-nav-item flex flex-col items-center gap-0.5 py-1.5 px-2 rounded-xl hover:bg-brand-50 group">
                <div class="w-9 h-9 rounded-full flex items-center justify-center transition-colors {{ request()->routeIs('user.dashboard') ? 'bg-brand-500' : 'bg-brand-100' }} group-hover:bg-brand-500">
                    <svg class="w-4 h-4 transition-colors {{ request()->routeIs('user.dashboard') ? 'text-white' : 'text-brand-600' }} group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                </div>
                <span class="text-[9px] font-semibold transition-colors {{ request()->routeIs('user.dashboard') ? 'text-brand-700' : 'text-gray-600' }} group-hover:text-brand-700">{{ __('Home') }}</span>
            </a>
            <a href="{{ route('user.meals') }}" class="mobile-nav-item flex flex-col items-center gap-0.5 py-1.5 px-2 rounded-xl hover:bg-brand-50 group">
                <div class="w-9 h-9 rounded-full flex items-center justify-center transition-colors {{ request()->routeIs('user.meals*') ? 'bg-brand-500' : 'bg-brand-100' }} group-hover:bg-brand-500">
                    <svg class="w-4 h-4 transition-colors {{ request()->routeIs('user.meals*') ? 'text-white' : 'text-brand-600' }} group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                </div>
                <span class="text-[9px] font-semibold transition-colors {{ request()->routeIs('user.meals*') ? 'text-brand-700' : 'text-gray-600' }} group-hover:text-brand-700">{{ __('Meals') }}</span>
            </a>
            <a href="{{ route('user.nutrition') }}" class="mobile-nav-item flex flex-col items-center gap-0.5 py-1.5 px-2 rounded-xl hover:bg-brand-50 group">
                <div class="w-9 h-9 rounded-full flex items-center justify-center transition-colors {{ request()->routeIs('user.nutrition*') ? 'bg-brand-500' : 'bg-brand-100' }} group-hover:bg-brand-500">
                    <svg class="w-4 h-4 transition-colors {{ request()->routeIs('user.nutrition*') ? 'text-white' : 'text-brand-600' }} group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                </div>
                <span class="text-[9px] font-semibold transition-colors {{ request()->routeIs('user.nutrition*') ? 'text-brand-700' : 'text-gray-600' }} group-hover:text-brand-700">{{ __('Nutrition') }}</span>
            </a>
            <a href="{{ route('user.orders') }}" class="mobile-nav-item flex flex-col items-center gap-0.5 py-1.5 px-2 rounded-xl hover:bg-brand-50 group">
                <div class="w-9 h-9 rounded-full flex items-center justify-center transition-colors {{ request()->routeIs('user.orders*') ? 'bg-brand-500' : 'bg-brand-100' }} group-hover:bg-brand-500">
                    <svg class="w-4 h-4 transition-colors {{ request()->routeIs('user.orders*') ? 'text-white' : 'text-brand-600' }} group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
                </div>
                <span class="text-[9px] font-semibold transition-colors {{ request()->routeIs('user.orders*') ? 'text-brand-700' : 'text-gray-600' }} group-hover:text-brand-700">{{ __('Orders') }}</span>
            </a>
            <a href="{{ route('user.settings') }}" class="mobile-nav-item flex flex-col items-center gap-0.5 py-1.5 px-2 rounded-xl hover:bg-brand-50 group">
                <div class="w-9 h-9 rounded-full flex items-center justify-center transition-colors {{ request()->routeIs('user.settings*') ? 'bg-brand-500' : 'bg-brand-100' }} group-hover:bg-brand-500">
                    <svg class="w-4 h-4 transition-colors {{ request()->routeIs('user.settings*') ? 'text-white' : 'text-brand-600' }} group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                </div>
                <span class="text-[9px] font-semibold transition-colors {{ request()->routeIs('user.settings*') ? 'text-brand-700' : 'text-gray-600' }} group-hover:text-brand-700">{{ __('Settings') }}</span>
            </a>
        </div>
    </div>
